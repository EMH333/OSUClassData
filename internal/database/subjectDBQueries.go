...
//calculate average subject GPA as well as the standard deviation of the subject GPA
func GetSubjectAvgStdGPA(db *sql.DB, id string, term string) (float64, float64, error) {
	var query = "SELECT AVG(ClassGPA), STD(ClassGPA) FROM Classes WHERE ClassIdentifier LIKE ? AND TermID=? AND Visible=TRUE GROUP BY TermID ORDER BY TermID"
